...
export class mysNews extends plugin {
    constructor(e) {
        super({
            ...
            rule: [
                {
                    //逻辑：只能以 游戏前缀或游戏名 开头
                    reg: "^(#+(官方|星铁|原神|崩坏三|崩三|绝区零|崩坏二|崩二|崩坏学园二|未定|未定事件簿)?|#*(官方|星铁|原神|崩坏三|崩三|绝区零|崩坏二|崩二|崩坏学园二|未定|未定事件簿))(公告|资讯|活动)(列表|[0-9])*$",
                    fnc: "news"
                },
                {
                    reg: "^(#米游社|#mys)(.*)",
                ...
                {
                    reg: "^#*(星铁|原神|崩坏三|崩三|绝区零|崩坏二|崩二|崩坏学园二|未定|未定事件簿)?(开启|关闭)(公告|资讯|活动)推送$",
                    fnc: "setPush"
                },
                {
                    reg: "^#(星铁|原神|崩坏三|崩三|绝区零|崩坏二|崩二|崩坏学园二|未定|未定事件簿)?推送(公告|资讯|活动)$",
                    permission: "master",
                    fnc: "mysNewsTask"
                },
                {
                ...
                {
                    reg: "^#?(星铁|崩铁|原神|崩坏三|崩三|绝区零)?(清空|清理|清除)(到期)?活动(推送)?(缓存|记录)?$",
                    permission: "master",
                    fnc: "clearActivityCache"
                }
            ]
        })
    ...
    async clearActivityCache() {
        // 解析指令里包含的是哪个游戏
        let gameMatch = this.e.msg.match(/(星铁|原神|崩坏三|崩三|绝区零)/)
        let gameName = gameMatch ? gameMatch[0] : ""

        // 映射到具体的 redis 简称
        let gameKey = "*" // 如果没写游戏名，默认清除所有
        if (gameName === "原神") gameKey = "gs"
        if (gameName === "崩铁" || gameName === "星铁") gameKey = "sr"
        if (gameName === "绝区零") gameKey = "zzz"
        if (gameName === "崩三" || gameName === "崩坏三") gameKey = "bbb"

        let keys = []
        if (this.e.isGroup) {
            keys = await redis.keys(`Yz:apgl:${gameKey}:*:${this.e.group_id}`)
        } else {
            keys = await redis.keys(`Yz:apgl:${gameKey}:*`)
        }

        if (keys.length === 0) {
            let target = gameName ? `[${gameName}] ` : ""
            await this.reply(`当前没有任何${target}到期活动推送缓存哦~`)
            return true
        }
        ...
        let cmd = gameName ? `#${gameName}推送活动` : "#推送活动"
        let msg = `清理成功！共删除了 ${keys.length} 条 ${gameName || "所有游戏"} 相关的活动推送记录。\n👉现在发送 ${cmd} 即可单独测试。`
        await this.reply(msg)
        return true
    }
}
