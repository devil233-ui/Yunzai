...
export default class Ledger extends base {
  ...
  dealData(ledgerInfo) {
    ...
    let iconChar = lodash.sample(this.e.isSr ? [ "希儿", "白露", "艾丝妲", "布洛妮娅", "姬子", "卡芙卡", "克拉拉", "停云", "佩拉", "黑塔", "希露瓦", "银狼" ] : [ "七七", "刻晴", "温迪", "芭芭拉", "诺艾尔", "重云", "丽莎", "北斗", "班尼特", "莫娜", "达达利亚", "阿贝多", "凝光", "可莉", "琴", "菲谢尔", "迪卢克", "雷泽", "凯亚", "安柏", "砂糖", "行秋", "迪奥娜", "香菱" ])
    let icon = this.e.isSr ? Character.get(iconChar, "sr").imgs?.face : `img/ledger/icon/${iconChar}.png`
  ...
  async ledgerCountHistory() {
    ...
    if (!NoteData || lodash.isEmpty(NoteData)) {
      this.e.reply(this.e?.isSr
        ? [ "暂无星琼数据，请先发送 *星琼", segment.button([ { text: "星琼", input: "*星琼" } ]) ]
        : [ "暂无原石数据，请先发送 #原石", segment.button([ { text: "原石", input: "#原石" } ]) ], false, { at: true })
      return false
    }
  ...
  ledgerCountData(NoteData, nowYear) {
    ...
    lodash.forEach(NoteData, (val) => {
      ...
      // 柱状图数据
      if (this.e.isSr) { val.data_month = val.data_month.slice(-2, -1) == "0" ? val.data_month.slice(-1) : val.data_month.slice(-2) }
