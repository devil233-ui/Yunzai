  ...
  async getBody() {
    ...
    if (this.dataCharacter) {
      ...
      if (this.e.isSr) {
        ...
        if (detail.data.skills_servant) this.isMemo = true
        skillList = this.isMemo ? [ ...detail.data.skills, ...detail.data.skills_servant ] : detail.data.skills
      } else {
      ...
      if (!this.e.isSr) {
        computes[i].forEach(({ consume }) => {
          consume.forEach(val => {
            ...
            val.lack_num = formart(val.lack_num)
